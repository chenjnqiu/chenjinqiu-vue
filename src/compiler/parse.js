import { createStringLiteral, createArrayExpression, createCallExpression } from './compileScript'
// 定义状态机的状态
const State = {
    initial: 1, // 初始状态
    tagOpen: 2, // 标签开始状态
        ...
        switch (currentState) {
            ...
            case State.tagName:
                if (isAlpha(char)) {
                    ...
                    // 2. 同时创建一个标签 Token，并添加到 tokens 数组中,注意，此时 chars 数组中缓存的字符就是标签名称
                    tokens.push({
                        type: 'tag',
                        name: chars.join('')
                    })
            ...
            case State.tagEndName:
                if (isAlpha(char)) {
                    ...
                    // 1. 遇到字符 >，切换到初始状态
                    currentState = State.initial
                    // 2. 从 结束标签名称状态 --> 初始状态，应该保存结束标签名称Token,注意，此时 chars 数组中缓存的内容就是标签名称
                    tokens.push({
                        type: 'tagEnd',
                        name: chars.join('')
                    })
...
export function transform(ast) {
    ...
    const context = {
        ...
        replaceNode(node) {
            // 为了替换节点，我们需要修改 AST
            // 找到当前节点在父节点的 children 中的位置：context.childIndex然后使用新节点替换即可
